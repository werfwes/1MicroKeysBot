        ...
        def text_message(message):
            if message.text.lower() == 'помощь⚙':
                ...
                msg = 'Категории: \n' \
                      '➖➖➖Windows➖➖➖\n' \
                      '    🔐Windows 10 Pro \n' \
                      '                  ➡' + str(win10ProPrice) + ' руб. | ' + str(
                    sum(1 for line in open('Win10ProKeys.txt', 'r'))) + ' шт.' + '\n' \
                                                                                 '    🔐Windows 10 Home \n' \
                                                                                 '                  ➡' + str(
                    win10HomePrice) + ' руб. | ' + str(
                    sum(1 for line in open('Win10HomeKeys.txt', 'r'))) + ' шт.' + '\n' \
                                                                                  '➖➖➖Office➖➖➖\n' \
                                                                                  '    🔐Office 2016 \n' \
                                                                                  '                  ➡' + str(
                    off16Price) + ' руб. | ' + str(sum(1 for line in open('Off16Keys.txt', 'r'))) + ' шт.' + '\n' \
                                                                                                             ''
                ...
                if sum(1 for line in open('Win10ProKeys.txt', 'r')) > 0:
                    ...
                    bot.send_message(message.chat.id, 'Для оплаты переведите ' + str(
                        win10ProPrice) + ' рублей на счет QIWI +79138653949 с комментарием ' + str(
                        random_code) + ' и нажмите на кнопку "Проверить оплату"', reply_markup=markup)
                ...
                if sum(1 for line in open('Win10HomeKeys.txt', 'r')) > 0:
                    ...
                    bot.send_message(message.chat.id,
                                     'Для оплаты переведите ' + str(win10HomePrice) + ' рублей на счет QIWI \n'
                                                                                      'Номер: +79138653949 \n'
                                                                                      'Kомментарий: ' + str(
                                         random_code) + '\n'
                                                        'Статус: Не оплачено', reply_markup=markup)
                else:
                ...
                if sum(1 for line in open('Off16Keys.txt', 'r')) > 0:
                    ...
                    bot.send_message(message.chat.id, 'Для оплаты переведите ' + str(
                        off16Price) + ' рублей на счет QIWI +79138653949 с комментарием ' + str(
                        random_code) + ' и нажмите на кнопку "Проверить оплату"', reply_markup=markup)
                else:
                    bot.send_message(message.chat.id, 'Нет в наличии, ожидайте пополнение :(')
                ...
                for i in range(len(req['data'])):
                    if req['data'][i]['comment'] == str(random_code):
                        if req['data'][i]['sum']['amount'] == sum:
                            ...
                            print('succesful')
                            # TODO: код, который сработает, если оплата прошла успешно
                            filename = ''
